...
class Solver(object):
    ...
    def train_and_eval(self):
            ...
            if os.path.exists(PATH):
                ...
                for i_batch, batch_data in enumerate(self.train_loader):
                    ...
                    with torch.no_grad():
                        ...
                        text_embedding[i_batch] = enc_word[:,0,:] # (batch_size, emb_size)
                        audio_embedding[i_batch] = acoustic_enc(audio, alens)
                ...
                for i_batch, batch_data in enumerate(self.train_loader):
                    ...
                    if optimizer_visual is not None:
                        if text_wait < 1.0:
                            ...
                            loss_tmp, nce_tmp, ba_tmp = optimize_(text_out, audio_embedding[i_batch], visual_embedding[i_batch], y, mem, optimizer_main, optimizer_text, model, text_enc, loss_tmp, nce_tmp, ba_tmp, stage)
                            text_wait = self.server_time+15/5
                            num_updates += 1
                        else:
                            text_wait -= 1.0
                        if audio_wait < 1.0:
                            ...
                            loss_tmp, nce_tmp, ba_tmp = optimize_(text_embedding[i_batch], audio_out, visual_embedding[i_batch], y, mem, optimizer_main, optimizer_text, model, acoustic_enc, loss_tmp, nce_tmp, ba_tmp, stage)
                            audio_wait = self.server_time
                            num_updates += 1
                        else:
                            audio_wait -= 1.0
                        if visual_wait < 1.0:
                            ...
                            loss_tmp, nce_tmp, ba_tmp = optimize_(text_embedding[i_batch], audio_embedding[i_batch], visual_out, y, mem, optimizer_main, optimizer_text, model, visual_enc, loss_tmp, nce_tmp, ba_tmp, stage)
                            visual_wait = self.server_time+15/10
                            num_updates += 1
                        else:
                            visual_wait -= 1.0
        ...
        def train(model, text_enc, acoustic_enc, visual_enc, optimizer, optimizer_visual, optimizer_acoustic, optimizer_text, criterion, epoch, stage=1):
            ...
            for i_batch, batch_data in enumerate(self.train_loader):
                ...
                if self.alg == 'sync': 
                    ...
                    local_epochs_audio = 20
                    local_epochs_visual = 20
                elif self.alg == 'sync1': 
                    local_epochs_main = 5 
                    local_epochs_text = 5
                    local_epochs_audio = 5
                    local_epochs_visual = 5
                elif self.alg == 'flex': 
                    local_epochs_main = 20
                    local_epochs_text = 5
                    local_epochs_audio = 15
                    local_epochs_visual = 10
                elif self.alg == 'pbcd': 
                    local_epochs_main = 1
                    local_epochs_text = 1
                    local_epochs_audio = 1
                    local_epochs_visual = 1
                else:
                    print("Invalid algorithm chosen:", self.alg)
                    return None
                loss_tmp = 0
                nce_tmp = 0
                ba_tmp = 0
        ...
        for epoch in range(self.start_epoch, self.hp.num_epochs+1):
            ...
            if self.alg != "vafl":
                train_loss = train(model, text_enc, acoustic_enc, visual_enc, optimizer_main, optimizer_visual, optimizer_acoustic, optimizer_text, criterion, epoch, 1)
            else:
                train_loss = train_vafl(model, text_enc, acoustic_enc, visual_enc, optimizer_main, optimizer_visual, optimizer_acoustic, optimizer_text, criterion, epoch, 1)
            ...
            if self.alg == "vafl":
                pickle.dump(text_embedding, open(f'text_embedding{self.suffix}.pkl', 'wb'))
                pickle.dump(audio_embedding, open(f'audio_embedding{self.suffix}.pkl', 'wb'))
                pickle.dump(visual_embedding, open(f'visual_embedding{self.suffix}.pkl', 'wb'))
