...
parser.add_argument('--mode', default="nonadapt", type=str,
        help='VFL algorithm to use: nonadapt or adapt')
...
def main_worker(index, args):
    ...
    rand_inds = np.random.randint(12476, size=(args.num_clients+1)).tolist()
    cpus = cpus[rand_inds]
    cpus[cpus < 0] = 0
    cpus[cpus > 1] = 1
    lrs = []
    local_epochs = [] 
    for client in range(args.num_clients+1):
        lrs.append(np.abs(args.lr/(10*np.max(1-cpus[client])+1)))
        local_epochs.append((10*np.max(1-cpus[client])+1).astype(int))
    print(local_epochs)
    print(lrs)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        loss, acc1, acc5, lrs, local_epochs = train(train_loader, models, criterion, optimizers, epoch, args, lrs, cpus, local_epochs)
...
def create_data_loaders(args):
    ...
    #train_dataset_sub = torch.utils.data.Subset(train_dataset, indices[:int(len(train_dataset)/64)])
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        for client in range(args.num_clients):
            ...
            if wait[client] < 1.0:
                ...
                if client < 10:
                    wait[client] += (10-client+1)+args.server_time 
                elif client == 1:
                    wait[client] += args.server_time
            else:
                wait[client] -= 1.0
...
def train(train_loader, models, criterion, optimizers, epoch, args, lrs, cpus, local_epochs_prev):
    batch_time = utils.AverageMeter('Time', '6.3f')
    data_time = utils.AverageMeter('Data', '6.3f')
    losses = utils.AverageMeter('Loss', '.4e')
    top1 = utils.AverageMeter('Acc1', '6.2f')
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        local_epochs = np.abs((1-cpus[:,(int(epoch*num_batches+i) % cpus.shape[1])])*10+1).astype(int)
        print(local_epochs)
        print(lrs)
        # Train clients and server for Q rounds
        for client in range(args.num_clients+1):
            optimizers[client] = torch.optim.SGD(models[client].parameters(), lrs[client], momentum=args.momentum)
    ...
    loss, acc1, acc5 = losses.avg, top1.avg, top5.avg
    print(f'Training * Loss {loss:.5f} Acc1 {acc1:.3f} Acc5 {acc5:.3f}')
    return loss, acc1, acc5, lrs, local_epochs_prev
